...
int MotionEngine_Init(MotionEngine_State* engine) {
    ...
    simple_calibrate(200, engine->gyro_bias, engine->accel_bias); 
...
void MotionEngine_Update(MotionEngine_State* engine) {
    const float dt = 0.1f; // 假设更新频率是 100Hz，依赖于定时器设置
    ...
    if (mpu9250_basic_read(engine->g, engine->dps, engine->ut) == 0) {
        ...
        engine->euler_angles[2] += 17.0f; // 磁偏角校正
    }
}

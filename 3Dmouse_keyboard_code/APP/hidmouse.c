...
#define MOUSE_REPORT_INTERVAL 10
#define MOUSE_ANGLE_DEAD_ZONE  30
...
uint16_t HidEmu_ProcessEvent(uint8_t task_id, uint16_t events)
{
    ...
    {
        // 1. 更新姿态数据。MotionEngine_Update 内部现在会自己搞定精确的 dt
        MotionEngine_Update(&mouse_motion);

        // 2. 将欧拉角映射为鼠标的X, Y移动 (这部分逻辑不变)
        float roll = mouse_motion.euler_angles[0];
        float pitch = mouse_motion.euler_angles[1];
        int8_t x_move = 0;
        int8_t y_move = 0;
        if (roll > MOUSE_ANGLE_DEAD_ZONE)
        {
             x_move = 5;
         }
        else if (roll < -MOUSE_ANGLE_DEAD_ZONE) {
            x_move = -5;
        }
        if (pitch > MOUSE_ANGLE_DEAD_ZONE)
        {
             y_move = 5;
         }
        else if (pitch < -MOUSE_ANGLE_DEAD_ZONE) {
            y_move = -5;
        }
        // PRINT("%s \n",&x_move);
        // PRINT("%s \n",&y_move);
        hidEmuSendMouseReport(MOUSE_BUTTON_NONE, x_move, y_move);

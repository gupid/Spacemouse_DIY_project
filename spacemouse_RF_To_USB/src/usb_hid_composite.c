...
const uint8_t MyCfgDescr[] = {
    ...
    // wTotalLength: 总长度 = 原HID(66) + 新CDC(66) = 132 (0x84)
    0x84, 0x00,
    // bNumInterfaces: 接口总数 = 键盘(0) + SpaceMouse(1) + CDC通信(2) + CDC数据(3) = 4个
    0x04,
...
// 检查键盘端点(EP1)是否就绪可以发送
bool usb_hid_is_ep1_ready(void)
{
    return (R8_UEP1_CTRL & MASK_UEP_T_RES) == UEP_T_RES_NAK;
}

// 检查SpaceMouse端点(EP2)是否就绪可以发送
bool usb_hid_is_ep2_ready(void)
{
    return (R8_UEP2_CTRL & MASK_UEP_T_RES) == UEP_T_RES_NAK;
}
...
void usb_hid_report_axes(int16_t trans[3], int16_t rot[3])
{
    ...
    memcpy(pEP2_IN_DataBuf, report_buf, 7); // 注意是 pEP2_IN_DataBuf
    DevEP2_IN_Deal(7); // 注意是 DevEP2_IN_Deal

    // 等待端点2就绪
    while((R8_UEP2_CTRL & MASK_UEP_T_RES) != UEP_T_RES_NAK);

    // 2. 发送旋转报告 (Report ID = 2)
    report_buf[0] = 0x02;
...
/**
 * @brief   [重构后] 发送单个SpaceMouse报告（平移或旋转）
 * @param   report_id - 报告ID (0x01 for Translation, 0x02 for Rotation)
 * @param   axis_data - 指向3个int16_t轴数据的指针。如果为NULL，则发送零值报告。
 * @note    此函数现在是完全非阻塞的。
 */
void usb_hid_send_spacemouse_report(uint8_t report_id, const int16_t* axis_data)
{
    // [修改] 移除所有阻塞的 while 循环，并简化逻辑

    uint8_t report_buf[7]; // 报告缓冲区 (1字节ID + 6字节数据)

    // 检查是否是有效的报告ID
    if (report_id == 0x01 || report_id == 0x02)
    {
        ...
        {
            // 如果提供了数据，则拷贝数据
            memcpy(&report_buf[1], axis_data, 6);
        }
        else
        {
            // 如果数据指针为NULL，则发送零值报告
            memset(&report_buf[1], 0, 6);
        }

        // 注意：调用者必须通过 usb_hid_is_ep2_ready() 确保端点就绪
